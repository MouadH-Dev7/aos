...
export default function AgencyDetails({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const [agencyData, wilayasData, communesData] = await Promise.all([
          fetchJson(`${accountBaseUrl}/agences/${targetId}/`),
        ...
        const listingsData = await fetchJson(
          `${listingBaseUrl}/properties/list/?user_id=${agencyData?.user_id || ""}`
        );
  ...
  }, [accountBaseUrl, listingBaseUrl, locationBaseUrl, agency?.id, agency?.user_id, agencyId]);
  ...
  const locationLabel = useMemo(() => {
    if (!currentAgency) return "Unknown location";
    const commune = communeById.get(String(currentAgency.commune_id));
    const wilaya = commune ? wilayaById.get(String(commune.wilaya_id)) : null;
    return (
      [commune?.name_en, wilaya?.name_en].filter(Boolean).join(", ") ||
      "Unknown location"
    );
  }, [currentAgency, communeById, wilayaById]);
          ...
          <>
            <section className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="rounded-2xl bg-white border border-slate-200 p-6 shadow-sm">
                    ...
                    <div className="h-28 w-28 rounded-2xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                      {currentAgency.logo_url ? (
                        <img
                          ...
                          alt={currentAgency.company_name}
                        />
                      ) : (
                        <span className="material-symbols-outlined text-slate-400 text-3xl">business</span>
                      )}
                    </div>
                    <h2 className="mt-4 text-2xl font-black">{currentAgency.company_name}</h2>
                    <p className="text-sm text-slate-500 mt-1">{currentAgency.owner_name}</p>
                  ...
                  <div className="mt-6 space-y-3 text-sm text-slate-600">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span>{currentAgency.owner_name || "Unknown owner"}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-primary text-base">badge</span>
                      <span>Registration #{currentAgency.registration_number || "-"}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="material-symbols-outlined text-primary text-base">mail</span>
                      <span>contact@agency.dz</span>
                    </div>
                    <div className="flex items-center gap-2">
              ...
              <div className="lg:col-span-8 space-y-6">
                <div className="rounded-2xl bg-white border border-slate-200 p-6 shadow-sm">
                  <h3 className="text-xl font-bold">About the Agency</h3>
                  <p className="mt-3 text-slate-600 leading-relaxed">
                    {currentAgency.description || "No description provided."}
                  </p>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  <div>
                    <h3 className="text-xl font-bold">Agency Listings</h3>
