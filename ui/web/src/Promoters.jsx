...
export default function Promoters({
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        setLoading(true);
        setError("");
        const [promotersData, listingsData] = await Promise.all([
          cachedFetchJson(`${accountBaseUrl}/promoteurs/`, { ttlMs: 60000 }),
          cachedFetchJson(`${listingBaseUrl}/properties/list/`, { ttlMs: 60000 }),
        ]);
        if (!alive) return;
        setPromoters(Array.isArray(promotersData) ? promotersData : []);
        ...
        if (!alive) return;
        setError(err?.message || "Failed to load promoters.");
      } finally {
  ...
  const enrichedPromoters = useMemo(() => {
    return promoters
      .map((promoter) => {
        const commune = communeById.get(String(promoter.commune_id));
        ...
        const listingsCount = listingsCountByUser.get(String(promoter.user_id)) || 0;
        const locationLabel =
  ...
  }, [promoters, communeById, dairaById, wilayaById, listingsCountByUser]);

  const filteredPromoters = useMemo(() => {
    const search = filters.search.trim().toLowerCase();
    return enrichedPromoters.filter((promoter) => {
      const matchesSearch = search
        ? [promoter.company_name, promoter.owner_name, promoter.description, promoter.locationLabel]
            .filter(Boolean)
            .some((value) => String(value).toLowerCase().includes(search))
        : true;
      const matchesWilaya = filters.wilayaId
        ? String(promoter.wilaya?.id || "") === String(filters.wilayaId)
        : true;
      return matchesSearch && matchesWilaya;
    });
  }, [enrichedPromoters, filters.search, filters.wilayaId]);

  useEffect(() => {
    setPage(1);
  }, [filters.search, filters.wilayaId]);

  const totalPages = Math.max(1, Math.ceil(filteredPromoters.length / pageSize));
  const safePage = Math.min(page, totalPages);
  const pagedPromoters = useMemo(() => {
    const start = (safePage - 1) * pageSize;
    return filteredPromoters.slice(start, start + pageSize);
  }, [filteredPromoters, pageSize, safePage]);
  ...
  return (
    <div className="bg-background-light dark:bg-background-dark font-display text-slate-900 dark:text-slate-100 min-h-screen">
      <main className="mx-auto max-w-7xl px-6 py-10 space-y-8">
        ...
        {!loading && !error && spotlightPromoter && (
          <section className="rounded-2xl border border-primary/20 bg-white p-6 shadow-sm">
            <p className="text-xs font-bold uppercase text-primary">Top Promoter</p>
            <div className="mt-3 flex flex-col md:flex-row gap-5 md:items-center">
              <div className="h-20 w-20 rounded-xl bg-slate-100 border border-slate-200 overflow-hidden flex items-center justify-center">
                {spotlightPromoter.logo_url ? (
                  <img
                    src={spotlightPromoter.logo_url}
                    alt={spotlightPromoter.company_name}
              ...
              <div className="flex-1">
                <h3 className="text-xl font-black">{spotlightPromoter.company_name}</h3>
                <p className="text-sm text-slate-500">Owner: {spotlightPromoter.owner_name}</p>
                <p className="text-sm text-slate-500">{spotlightPromoter.locationLabel}</p>
              </div>
              <div className="text-right">
                <p className="text-xs uppercase text-slate-400 font-bold">Active Listings</p>
                <p className="text-2xl font-black text-primary">{spotlightPromoter.listingsCount}</p>
                <button
                  className="mt-3 rounded-lg bg-primary px-3 py-1.5 text-xs font-bold text-white"
                  type="button"
                  onClick={() => onViewPromoter?.(spotlightPromoter)}
                >
                  View Promoter
                </button>
              </div>
        ...
        {!loading && !error && (
          <section>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-xl font-bold">All Promoters</h3>
              <span className="text-sm text-slate-500">{filteredPromoters.length} result(s)</span>
            </div>
                ...
                {pagedPromoters.map((promoter) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h4 className="font-bold">{promoter.company_name}</h4>
                        <p className="text-xs text-slate-500">Owner: {promoter.owner_name}</p>
                      </div>
                    </div>
                    <p className="mt-3 text-sm text-slate-500">{promoter.locationLabel}</p>
                    <p className="mt-2 text-sm text-slate-600 line-clamp-2">
                      {promoter.description || "No description."}
                    </p>
                    <div className="mt-4 flex items-center justify-between text-sm">
                      <span className="text-slate-500">Listings</span>
                      <span className="font-bold text-primary">{promoter.listingsCount}</span>
                    </div>
                    <button
