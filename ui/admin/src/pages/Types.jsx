...
export default function AdminTypes({
  ...
  const normalized = useMemo(
    () => types.map((item) => ({ ...item, name: String(item.name || "").trim() })),
    [types]
  );
  ...
  const loadTypes = async () => {
    ...
    try {
      ...
      const response = await fetchWithFallback(ADMIN_BASE_URLS, "/types/", {
        headers: { Authorization: `Bearer ${access}` },
      });
      const data = await response.json();
      if (!response.ok) {
        throw new Error(data?.detail || "Failed to load types");
      }
  ...
  const addType = async (event) => {
    ...
    try {
      ...
      const data = await response.json();
      if (!response.ok) {
        const apiError = data?.name?.[0] || data?.detail || "Failed to add type";
        throw new Error(apiError);
      }
      setTypes((prev) => [...prev, data].sort((a, b) => Number(a.id) - Number(b.id)));
      setNewName("");
  ...
  const saveType = async () => {
    ...
    try {
      const access = localStorage.getItem("admin_access");
      const response = await fetchWithFallback(ADMIN_BASE_URLS, `/types/${editingId}/`, {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${access}`,
      ...
      const data = await response.json();
      if (!response.ok) {
        const apiError = data?.name?.[0] || data?.detail || "Failed to update type";
        throw new Error(apiError);
      }
      setTypes((prev) =>
  ...
  const removeType = async (typeId) => {
    ...
    try {
      const access = localStorage.getItem("admin_access");
      const response = await fetchWithFallback(ADMIN_BASE_URLS, `/types/${typeId}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${access}` },
      });
      if (!response.ok) {
      ...
      setTypes((prev) => prev.filter((item) => Number(item.id) !== Number(typeId)));
      ...
      <div className="flex min-h-screen overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 z-10">
            <div>
              <h1 className="text-lg font-bold">Types Management</h1>
              <p className="text-xs text-slate-500">Create, update, and remove property types</p>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-8 space-y-6 bg-slate-50/50 dark:bg-background-dark/50">
            <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              <h2 className="text-base font-bold mb-4">Add New Type</h2>
              <form className="flex flex-col sm:flex-row gap-3" onSubmit={addType}>
                <input
                  className="h-11 flex-1 rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm"
                  placeholder="Type name (e.g. Apartment)"
                  value={newName}
            ...
            <section className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
              <div className="border-b border-slate-100 px-6 py-4 flex items-center justify-between">
                <h2 className="text-base font-bold">All Types</h2>
                <button
